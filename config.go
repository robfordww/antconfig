...
type AntConfig struct {
    ...
    // When empty, SetValues will fall back to os.Args[1:].
    FlagArgs   []string
    ...
    FlagPrefix string
    // FlagSet, if provided, will be populated via BindFlags and consulted for parsed values.
...
func (c *AntConfig) SetFlagPrefix(prefix string) { c.FlagPrefix = prefix }

// SetFlagSet sets the flagset to be used/read when applying flag overrides.
func (c *AntConfig) SetFlagSet(fs *flag.FlagSet) { c.FlagSet = fs }

// BindFlags registers flags for all fields tagged with `flag:"name"` onto the provided FlagSet.
// It respects AntConfig.FlagPrefix for the CLI names. This method does not parse or apply flags;
// call fs.Parse(...) yourself, then SetValues to apply.
func (a *AntConfig) BindFlags(fs *flag.FlagSet, cfg any) error {
    fields, err := findFieldsWithTag("flag", cfg)
    if err != nil {
        return err
    }
...
func (a *AntConfig) SetValues(c any) error {
...
func processFlags(fieldList []fieldWithTagValue, args []string, prefix string) error {
    ...
    for i := 0; i < len(args); i++ {
        ...
        var valStr *string
        // handle --name=value
        if eq := indexByte(keyAndMaybe, '='); eq >= 0 {
            key = keyAndMaybe[:eq]
            v := keyAndMaybe[eq+1:]
            valStr = &v
        } else {
    ...
    for _, row := range fieldList {
        ...
        if !ok || valPtr == nil {
            if prefix != "" {
                if v2, ok2 := values[prefix+name]; ok2 && v2 != nil { valPtr = v2 } else { continue }
            } else {
                continue
            }
        }
