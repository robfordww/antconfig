...
func TestPrecedence_ConfigEnvDotenvFlags(t *testing.T) {
    ...
    if err := os.WriteFile(cfgPath, cfgContent, 0644); err != nil { t.Fatal(err) }
    ...
    var cfg Cfg
    ant := &AntConfig{}
    if err := ant.SetConfigPath(cfgPath); err != nil { t.Fatalf("SetConfigPath: %v", err) }
    if err := ant.SetEnvPath(envPath); err != nil { t.Fatalf("SetEnvPath: %v", err) }
    ...
    if err := ant.SetValues(&cfg); err != nil { t.Fatalf("SetValues (flags): %v", err) }
